...
// MarkBookingPaid: manual update status to paid (for testing/demo)
func (h *BookingHandler) MarkBookingPaid(c *fiber.Ctx) error {
        ...
        // Transaction to update booking and mark seats as booked (permanent)
        err = dbConn.Transaction(func(tx *gorm.DB) error {
                if err := repoBooking.SimpanUpdate(tx, booking); err != nil {
                        return err
                }

                // Update ketersediaan kursi status to 'booked'
